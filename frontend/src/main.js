    ...
    try {
        ...
        if (error.message === "Failed to fetch") {
            $("#errStart").textContent =
                "Cannot connect to server. Make sure the backend is running on http://localhost:5050";
        } else {
...
function renderMain() {
    ...
    $("#dayLabel").textContent = `Day ${gameState.currentDayIndex + 1}`;
...
$("#btnHold").addEventListener("click", async () => {
    try {
        ...
        // Check if game ended
        if (data.message === "Game ended. No more data available." || data.gameOver) {
            await finishGame();
        } else {
            renderMain();
        }
...
$("#btnQuit").addEventListener("click", async () => {
    if (gameState.currentDayIndex <await 6) {
        alert("You can quit after Day 7");
        return;
    }
...
$("#btnConfirm").addEventListener("click", async () => {
    ...
    try {
        ...
        // Check if game ended
        if (
            nextDayData.message === "Game ended. No more data available." || nextDayData.gameOver
        ) {
            await finishGame();
        } else {
            renderMain();
